...
class Perso {
    ...
    move() {
        document.addEventListener('keydown', (event) => {
            ...
            if ((code == "KeyD" || code == "ArrowRight") && this.mort == false && this.obstacle == false) {
                this.right = true;
                this.left = false;
                ctx.clearRect(this.posX, this.posY, cnv.width, cnv.height);
                this.posX += 10;
                this.polygon.translate(10, 0);
            ...
            else if ((code == "KeyA" || code == "ArrowLeft") && this.mort == false) {
                this.obstacle = false;
                this.left = true;
                this.right = false;
                ctx.clearRect(this.posX, this.posY, cnv.width, cnv.height);
...
function creaCaisse() {
    let x = [530, 730, 830, 930, 1030, 1130, 1230, 1430, 1630];
    let pos = [];


    for (let i = 0; i < 9; i++) {
        let r_x = Math.floor(Math.random() * (10 - 0) + 0);
        pos[i] = x[r_x];
    }
...
function collision() {
    for (let j = 0; j < ennemies.length; j++) {
        ...
        for (let k = 0; k < caisse_li.length; k++) {
            if (perso.collision(caisse_li[k])) {
                if (perso.posX < caisse_li[k].posX) {
                    perso.obstacle = true;
                }
            }
